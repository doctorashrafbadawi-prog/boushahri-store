...
const KWD = new Intl.NumberFormat("en-KW", {
  style: "currency",
  currency: "KWD",
...
export default function OrderForm({ supabase, user, onLogout }) {
  ...
  async function loadProducts() {
    ...
    const { data, error } = await supabase
      .from("products")
      .select("id, name, category, stock, sell_price")
    ...
    setItems((old) =>
      old.map((item, i) => {
        ...
        if (field === "product_id") {
          ...
          next.price = productPrice(product);
        }
  ...
  function removeItem(index) {
    setItems((old) =>
      old.length === 1 ? old : old.filter((_, i) => i !== index)
    );
  }
  ...
  async function sendRequest() {
    ...
    const validItems = items.filter(
      (item) => item.product_id && Number(item.qty) > 0
    );
    ...
    const requestPayload = {
      ...
      status: "Pending",
      total: grandTotal,
      notes: `Request submitted by ${user?.username || "clinic user"}`,
    };
    ...
    if (reqError) {
      ...
      setSending(false);
      setError("Could not send request. Please check database connection.");
      return;
    }
    ...
    if (itemsError) {
      ...
      setSending(false);
      setError("Request was created but items could not be saved.");
      return;
    }
    ...
    <div className="pageShell">
      ...
      <section className="orderCard">
        ...
        <div className="notice">
          Please review product quantities carefully before sending the request.
        </div>
        ...
        <button className="addBtn" onClick={addItem} type="button">
          + Add Product
        </button>

        <div className="grandTotal">
          <span>Grand Total</span>
          <b>{KWD.format(grandTotal)}</b>
        </div>
